...
def test_read_range(dut):
    """ Simple reading test for value range.
        ...
        TODO:
        1. What goes wrong? comparing string to int, remove whitespace
        2. Fix it. 
...
def test_invalid_values(dut):
    ...
    # Insert wanted test values to commands array
    commands = ["1234\r", " 1234\r", "4321\r", "test\r",
                "0est\r","tes1\r", "01234\r", "012345678\r",
                "0\r", "100\r", "500\r", "1000\r", "2000\r"]
    
    for command in commands:
        if is_valid(command):   
            my_interface.write(command)
            value = read_value(my_interface)
            # Checking if readed value is same that was sent
            if remove_whitespace(command) != remove_whitespace(value):
                results.append("FAIL")
            else:
                results.append("PASS")
        else:
            print("Error: Trying to write invalid value\n") #if invalid value is recognized PASS test
            results.append("PASS") #Add test result to results array
        sleep(1)
...
def test_measure(dut):
    ...
    test_values = [50, 100, 500, 800, 1000, 1250, 1500, 2000]
    lastValue = 0
    lastDc = 0

    open()  # Open voltmeter connection to WaveForms
    setup_acquisition()
    
    # test loop
    for test_value in test_values:
        write_value(my_interface, test_value)
        
        dc = meas_vdc(0)
        print("DC: "+ str(round(dc,4))+"V")

        value = read_value(my_interface)
        value = int(remove_whitespace(value))

        # test logic:
        #if read value = write value and measure dc value is smaller or bigger
        #than last value like in measured uart value
        if (value == test_value and value > lastValue and dc > lastDc):
            results.append("PASS")
        elif (value == test_value and value < lastValue and dc < lastDc):
            results.append("PASS")
        else:
            results.append("FAIL")
        sleep(1)
        
    close() #Close connection to WaveForms
    # end test content
...
def main():
    ...
    serial_port = "COM8"     # serial_port = "/dev/ttyUSB0"
